...
# 0) Load data
# ─────────────────────────────────────────────────────────────────────────────
df_all = pd.read_csv("/Users/milton/Desktop/qc_metrics_all_sites_func.tsv", sep="\t")

# only plot for echo 1
df_all = df_all[df_all['echo'] == 1 ]
...
df_all["bids-id"] = (
    ...
    + df_all["task"].fillna("").apply(lambda x: f"_task-{x}" if x else "")
...
id_vars = ["site", "bids-id", "echo", "run", "subject", "task"]
metric_columns = [
    "dvars_nstd", "dvars_std", "dvars_vstd", "efc", "fber", "fd_mean",
    "fd_num", "fd_perc", "fwhm_avg", "fwhm_x", "fwhm_y", "fwhm_z",
    "gsr_x", "gsr_y", "snr", "tsnr", "summary_bg_mean", "summary_fg_mean",
    "summary_bg_stdv", "summary_fg_stdv"
]
