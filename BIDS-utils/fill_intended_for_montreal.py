...
def fill_b0_meta(
    ...
    **bids_entities: Dict[str, Any],
    ...
    for epi in epis:
        ...
        print(fmap1,    match_fmap)
        if epi_series_id not in match_fmap.entities.get("B0FieldIdentifier", []):
            fmaps_append_b0tags.setdefault(get_json(match_fmap), set()).add(
                epi_series_id
            )
        if epi_series_id not in fmap1.entities.get("B0FieldIdentifier", []):
