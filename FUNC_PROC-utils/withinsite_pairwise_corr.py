...
site_configs = {
    ...
    "toronto": {
        ...
        "fmriprep_dir": "/data/debug-proc/CPIP_fMRI_July042025/CPIP_fMRI_July042025/derivatives/fmriprep/",
...
# Example:
# all_corr_df = compute_pairwise_roi_correlations_all(df, parcellation_labels)
# all_corr_df.head()

# Calculate all the posible correlations
# all_corr_df = compute_pairwise_roi_correlations_all(ll_calgary, parcellation_labels)

# load the mega timeseries with motion outliers indicators
...
site_configs["toronto"]["parcorr_ll"] = df["site"].eq("toronto") & df["task"].isin(["laluna"]) & df["run"].eq(2)
site_configs["toronto"]["parcorr_pc"] = df["site"].eq("toronto") & df["task"].isin(["partlycloudy"]) & df["run"].eq(1)
...
for site in site_configs:
    for acq in acq_masks:
        #get the df
        df_acq = df.loc[site_configs[site][acq]]
        # correlation = compute_pairwise_roi_correlations_all(df_acq, parcellation_labels, within=("subject","session"))
        run_corr_df = compute_pairwise_roi_correlations_all(
                        df_acq, parcellation_labels, within=("site","task","session")
                    )

        run_corr_df.to_csv(f"/data/debug-proc/corr_{site}_{acq}.tsv",sep="\t")
