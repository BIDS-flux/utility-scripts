...
# what are the tasks to be run
tasks = ["ll","pc"]

for task in configs:
    #get the df
    corr_df = _read_tsv_max_precision(f"/data/debug-proc/across_sites_corr_{task}.tsv")
    logger = setup_plot_logging(logging.INFO, to_file="/data/debug-proc/across_sites_corr_overlay.log")
    ...
    saved = plot_pairwise_correlation_overlays(
        ...
        out_dir=f"/data/debug-proc/across_sites_corr_figs_{task}",
        within=("task","session"),                         # groups that define “same acquisition”
        vmax=0.5,                                                 # scale [-1, 1]
